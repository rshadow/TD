...
#        $self->{img}{$item}->display_format;
        ...
        $self->{size}{$item}{normal} = SDL::Rect->new(
            -left  => 0,            -top => 0,
            -width => $width,       -height => $height);
        $self->{size}{$item}{current} = SDL::Rect->new(
            -left  => $width,       -top => 0,
            -width => $width,       -height => $height);
        $self->{size}{$item}{pressed} = SDL::Rect->new(
            -left  => $width * 2,   -top    => 0,
            -width => $width,       -height => $height);
...
sub draw
{
    ...
    # Draw background
    $self->bg_img->blit($self->bg_size, $self->app, $self->bg_dest);
    ...
    for my $index (0 .. $#{$self->model->items})
    {
        ...
        $self->{img}{$item}->blit(
            $self->{size}{$item}{normal}, $self->app, $self->{dest}{$item});
...
sub bg_img     {return shift()->{img}{background}}
sub bg_size    {return shift()->{size}{background}}
sub bg_dest    {return shift()->{dest}{background}}
